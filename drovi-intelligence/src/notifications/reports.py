...
def _is_french(locale: str | None) -> bool:
    return bool((locale or "").strip().lower().startswith("fr"))


def _localized_report_title(report_type: str, report_title: str, locale: str | None) -> str:
    if not _is_french(locale):
        return report_title

    # Best-effort localization for known reports. Keep provided title as fallback.
...
def _render_report_html(
    *,
    ...
    intro = "Genere par Drovi Intelligence." if _is_french(locale) else "Generated by Drovi Intelligence."
    return (
        f"<h2>{html.escape(header)}</h2>"
        f"<p>{html.escape(intro)}</p>"
        f"<pre style=\"font-family: ui-sans-serif, system-ui; white-space: pre-wrap;\">{safe_summary}</pre>"
    )
...
async def send_report_email(
    *,
    ...
    # Resolve org metadata for localization.
    ...
    if not organization_name:
        async with get_db_session() as session:
            ...
            row = meta.fetchone()
            if row:
                organization_name = getattr(row, "name", None) or organization_name
                org_locale = getattr(row, "default_locale", None) or org_locale

    localized_title = _localized_report_title(report_type, report_title, org_locale)

    subject = localized_title
    if organization_name:
        subject = f"{organization_name} — {localized_title}"
