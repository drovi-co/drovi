...
export function SourcesPage() {
  ...
  const patchConnectionInCache = useCallback(
    (
      connectionId: string,
      patch:
        | Partial<OrgConnection>
        | ((current: OrgConnection) => Partial<OrgConnection>)
    ) => {
      queryClient.setQueryData<OrgConnection[]>(
        orgConnectionsQueryKey,
        (current) => {
          ...
          const next = current.map((conn) => {
            ...
            changed = true;
            const resolvedPatch =
              typeof patch === "function" ? patch(conn) : patch;
            return { ...conn, ...resolvedPatch };
          });
          return changed ? next : current;
        }
      );
    },
    [orgConnectionsQueryKey, queryClient]
  );

  const removeConnectionFromCache = useCallback(
    (connectionId: string) => {
      queryClient.setQueryData<OrgConnection[]>(
        orgConnectionsQueryKey,
        (current) => (current ?? []).filter((conn) => conn.id !== connectionId)
      );
    },
    [orgConnectionsQueryKey, queryClient]
  );
  ...
  useEffect(() => {
    if (connectionSuccess === "success") {
      ...
      queryClient.invalidateQueries({
        queryKey: orgConnectionsQueryKey,
        exact: true,
      });
    } else if (error) {
      toast.error(
  ...
  }, [connectionSuccess, error, orgConnectionsQueryKey, queryClient, t]);
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (_result, connectionId) => {
      ...
      patchConnectionInCache(connectionId, {
        live_status: "started",
        progress: 0,
      });
    },
  ...
  const backfillMutation = useMutation({
    ...
    onSuccess: (_result, payload) => {
      ...
      patchConnectionInCache(payload.connectionId, {
  ...
  const visibilityMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      patchConnectionInCache(result.connection_id, {
        visibility: result.visibility as "private" | "org_shared",
      });
    },
    onError: (error: Error) => {
      toast.error(
