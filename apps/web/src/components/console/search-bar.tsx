...
function FilterChip({ entity, values, negated, onRemove }: FilterChipProps) {
  const displayValue = values.length === 1 ? values[0] : `${values.length} values`;
...
export const ConsoleSearchBar = forwardRef<ConsoleSearchBarRef, ConsoleSearchBarProps>(
  function ConsoleSearchBar(
    ...
    const needsDynamicValues = Boolean(
      cursorContext.mode === "value" &&
        ...
        ENTITY_DEFINITIONS.find((e) => e.name === cursorContext.entity)?.values ===
          "dynamic"
    );
    ...
    const suggestions = useMemo(() => {
      ...
      if (cursorContext.mode === "empty") {
        ...
        results.push(...ENTITY_DEFINITIONS.slice(0, 6).map((e) => ({
        ...
        results.push(...getValueSuggestions(cursorContext.entity, cursorContext.partial));
      ...
      (newValue: string) => {
        ...
        if (pendingEntity) {
          ...
          setCursorContext({
            ...
            partial: currentPart,
            existingValues: parts.slice(0, -1).map((p) => p.trim()).filter(Boolean),
          });
        } else {
    ...
    const handleSelect = useCallback(
      ...
      [pendingEntity, cursorContext, value, onChange, onFilterComplete, onTimeComplete]
    );
    ...
    const handleKeyDown = useCallback(
      ...
      [handleSubmit, handleSelect, open, allSuggestions, selectedIndex, pendingEntity, value, completeFilterWithTypedValue, addValueToPending, filters, timeRange, onTimeRemove, onFilterRemove]
    );
          ...
          >
            <div className="max-h-[300px] overflow-y-auto p-1">
              ...
              {cursorContext.mode === "empty" && (
                <div className="px-2 py-1.5 font-medium text-[11px] uppercase tracking-wider text-muted-foreground">
                  Suggested Filters
                </div>
              )}
...
export function useSavedSearches(_organizationId: string) {
  ...
  return {
    ...
    saveSearch: (_name: string, _query: string) => {},
    deleteSearch: (_id: string) => {},
  };
}
