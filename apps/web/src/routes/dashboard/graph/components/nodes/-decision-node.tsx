...
import { Sparkles, Check, RotateCcw, Calendar } from "lucide-react";
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            {!nodeData.isSuperseded && (
              <div className="absolute -inset-1 rounded-2xl bg-gradient-to-r from-purple-500/20 to-fuchsia-500/20 blur-sm opacity-0 transition-opacity group-hover:opacity-100" />
            )}

            {/* Main card - hexagonal feel with rounded corners */}
            <div
              className={`
                relative flex min-w-[180px] max-w-[220px] flex-col gap-2 rounded-2xl border-2 bg-card p-3 shadow-lg transition-all
                ${nodeData.isSuperseded
                  ? "border-muted-foreground/20 bg-muted/50"
                  : "border-purple-400/50 bg-gradient-to-br from-purple-500/5 to-fuchsia-500/5"}
                ${selected ? "ring-4 ring-purple-500/20 border-purple-500" : ""}
                group-hover:shadow-xl group-hover:border-purple-400
              `}
            >
              {/* Decorative corner accents */}
              <div className="absolute -left-px -top-px h-4 w-4 rounded-tl-2xl border-l-2 border-t-2 border-purple-500/50" />
              <div className="absolute -right-px -top-px h-4 w-4 rounded-tr-2xl border-r-2 border-t-2 border-purple-500/50" />
              ...
              <div className="flex items-start gap-2">
                ...
                <div
                  ...
                  `}
                >
                  <Sparkles className={`h-4 w-4 ${nodeData.isSuperseded ? "text-muted-foreground" : "text-white"}`} />
                </div>
